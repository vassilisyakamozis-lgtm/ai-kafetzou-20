...
const Cup = () => {
  ...
  const onSubmit = async (data: CupReadingForm) => {
    // Frontend validation για το image
    if (!selectedImage) {
      toast({
  ...
  const getCupReading = async (formData: CupReadingForm, imageFile: File): Promise<string | null> => {
    ...
    // ✅ ΠΕΡΝΑΜΕ gender & ageRange στο API για να επηρεάζουν το prompt/ύφος
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              {/* ✅ ΝΕΟ: Φύλο & Ηλικιακό εύρος (πριν τον Τομέα Ενδιαφέροντος) */}
              <Card>
                <CardHeader>
                  <CardTitle className="text-mystical-purple">Στοιχεία Προφίλ</CardTitle>
                  <CardDescription>Προσαρμόζουν το ύφος και το περιεχόμενο της ανάγνωσης.</CardDescription>
                </CardHeader>
                <CardContent>
