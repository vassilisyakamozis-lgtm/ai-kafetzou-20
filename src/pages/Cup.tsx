...
export default function Cup() {
  ...
  const onSubmit = async (values: CupForm) => {
    ...
    // Αν έχεις URL εικόνας από storage, χρησιμοποίησε το (διαφορετικά μπορείς να ανεβάσεις πρώτα την File στο storage)
    // Για demo: δεν ανεβάζουμε, απλά δεν στέλνουμε image_url.
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("reading", {
        body: {
          ...
          image_url, // ή δώσε URL αν το έχεις
          // ΝΕΑ:
      ...
      if (data?.ok) {
        ...
        // Εδώ δείξε αποτέλεσμα όπως θες (modal/σελίδα)
